...
def downloadservice2x(hostedFeatureService, agsService, baseURL, whereClause, agsFeatures, agsTable, username, password, outputFC, getAttachments, cwd):
    #Variables
...
##    cwd = arcpy.GetParameterAsText(9)
    ...
    def addDomainsHosted(fc):
        ...
        req = urllib2.Request(baseURL[:-6], urllib.urlencode(params))
        ...
        try:
            if len(data['types'][0]['domains']) > 0:
                try:
                    ...
                    if desc.workspaceFactoryProgID == 'esriDataSourcesGDB.SdeWorkspaceFactory.1' or desc.workspaceFactoryProgID == 'esriDataSourcesGDB.FileGDBWorkspaceFactory.1':
                        if len(data['types'][0]['domains']) > 0:
