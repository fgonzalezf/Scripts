...
# -*- coding: iso-8859-1 -*-
#-------------------------------------------------------------------------------
# Name:        Actualización de Poligonos geograficos en base a Vista de tabla
# Author:      fgonzalezf
# Created:     21/11/2015
...
arcpy.env.workspace=r"Database Connections\METG_ODA_PROD.sde"
tabla = r"Database Connections\METG_ODA_PROD.sde\METG.V_CON_METADATA"
Puntos=r"Database Connections\METG_ODA_PROD.sde\METG.Metadato_Geografico\METG.METADATO_POL"
...
NombreArchivo =str(ahora.year)+str(ahora.month) +str(ahora.day) + str(ahora.hour) +str(ahora.minute)+str(ahora.second)+ "POL"+".txt"
Fileprj = open(pathlogs + os.sep+ NombreArchivo, "w")

fields = ["MD_METADATA_ID","FILEIDENTIFIER","TITLE","CI_DATE_ID","REFDATE","MD_IDENTIFICATION_ID","DENOMINATOR","WESTBOUNDLONGITUDE","EASTBOUNDLONGITUDE","SOUTHBOUNDLATITUDE","NORTHBOUNDLATITUDE","NORTE","ESTE","URL","EPOCA","TEMATICA","ESCALA"]
fields2 = ["MD_METADATA_ID","FILEIDENTIFIER","TITLE","CI_DATE_ID","REFDATE","MD_IDENTIFICATION_ID","DENOMINATOR","WESTBOUNDLONGITUDE","EASTBOUNDLONGITUDE","SOUTHBOUNDLATITUDE","NORTHBOUNDLATITUDE","NORTE","ESTE","URL","EPOCA","TEMATICA","ESCALA","SHAPE@"]
...
edit.startOperation()
print "Actualizando..........."
with arcpy.da.UpdateCursor("in_memory/tableInMemory", fields) as cursor1:
    for row1 in cursor1:      
        Y=Y+1
        ...
        pnt3 = arcpy.Point()
        pnt4 = arcpy.Point()
        with arcpy.da.UpdateCursor(Puntos, fields2 ,rowexpresion) as cursor2:
            X=0
            for row2 in cursor2:
                X=X+1
                #print X
                try:
                    if row2[0]== row1[0]:
                        ...
                        lineArray.add(pnt3)
                        lineArray.add(pnt4)                      
                        row2[17]=arcpy.Polygon(lineArray)
                        lineArray.removeAll()
            ...
            if X==0:
                ...
                try:
                    ...
                    #arcpy.Delete_management("layerApp")
                    cursor3 = arcpy.da.InsertCursor(Puntos, fields2)
                    row3=[]
                    row3.append(row1[0])
                    row3.append(str(row1[1]).encode('iso-8859-1'))
                    row3.append(str(row1[2]).encode('iso-8859-1'))
                    row3.append(row1[3]if row1[3] else None)
                    row3.append(row1[4]if row1[4] else None)
                    row3.append(row1[5])
                    row3.append(row1[6])
                    row3.append(row1[7])
                    row3.append(row1[8])
                    ...
                    lineArray.add(pnt3)
                    lineArray.add(pnt4)                      
                    row3.append(arcpy.Polygon(lineArray))
                    
                    cursor3.insertRow(row3)
                    del cursor3
                    lineArray.removeAll()
...
with arcpy.da.UpdateCursor(Puntos, fields) as cursor1:
    for row1 in cursor1:
        ...
        print "Registro... " + str(Y)
        rowexpresion= "MD_METADATA_ID ="+str(row1[0])
        with arcpy.da.UpdateCursor("in_memory/tableInMemory", fields ,rowexpresion) as cursor2:
            X=0
            #print X
            for row2 in cursor2:
                X=X+1
