...
def exportar (db, xmlFile,type):
    ruta=pathArcgis()
    esriGeodatabase= GetModule(ruta +os.sep+"esriGeoDatabase.olb")
    esriSystem=GetModule(ruta +os.sep+"esriSystem.olb")
    esriGeoadatabaseDistributed=GetModule(ruta +os.sep+"esriGeoDatabaseDistributed.olb")
    esriDataSourcesGDB=GetModule(ruta +os.sep+"esriDataSourcesGDB.olb")
    if (type== True):
...
def AutoCreateAnnot (FeatRuta):
    ruta=pathArcgis()
    esriGeodatabase= GetModule(ruta +os.sep+"esriGeoDatabase.olb")
    esriGeoprocessing= GetModule(ruta +os.sep+"esriGeoprocessing.olb")
    esriCarto= GetModule(ruta +os.sep+"esriCarto.olb")
...
def seleccion(Entrada,FeatSelect,Salida,workspace):
    ...
    result = int(arcpy.GetCount_management("Entrada_lyr").getOutput(0))
    if result !=0:
                ...
                if result>0:
                    ...
                    if(desc.featureType!="Annotation"):
                        ...
                        if (LinkAnnotation=="true"):
                            ...
                            fieldmapping.addTable(GeodatabaseEntrada+os.sep+dataset+os.sep+fc)
